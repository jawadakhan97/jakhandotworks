...
def get_git_commit_info(commit_hash=None):
    ...
    if commit_hash:
        ...
        # Try HEAD~1 first, fall back to just HEAD if no previous commit exists
        try:
            cmd = ["git", "rev-parse", "HEAD~1"]
            subprocess.run(cmd, capture_output=True, text=True, check=True)
            cmd = ["git", "diff", "--name-only", "HEAD~1", "HEAD"]
        except subprocess.CalledProcessError:
            # Only one commit exists, use git show instead
...
def detect_content_type(changed_files, force_type=None):
    ...
    Returns tuple: (content_type, file_path, issue_number, title)
    content_type can be: 'newsletter', 'blog', or None
    
    If force_type is specified, only look for that content type.
    """
    if force_type:
        pattern = re.compile(rf"src/content/{force_type}/(\d+)_(.+?)/index\.md$")
    else:
            ...
            if full_path.exists():
                ...
                if front_match:
                    try:
                        ...
                        return content_type, full_path, issue_num, title
                    except yaml.YAMLError:
                        pass
                
                return content_type, full_path, issue_num, f"Issue {issue_num}"
...
def render_template(template_path, variables):
    ...
    # Handle simple conditionals {% if var %}...{% endif %}
...
def build_newsletter_html(meta, body, template_path):
    ...
    variables = {
        ...
        "unsubscribe_url": "{{ unsubscribe_url }}",  # Kit will replace this
    }
...
def broadcast_to_kit(subject, html_content, subscriber_form_id=None):
    ...
    Kit API v2 Documentation:
    https://developers.kit.com/v2/reference/create-a-broadcast
    """
...
def main():
    ...
    # Detect all matching content files (not just the first one)
    results = []
    for file_path in changed_files:
        content_type, full_path, issue_num, title = detect_content_type([file_path], force_type)
        if content_type and full_path:
            results.append((content_type, full_path, issue_num, title))
    ...
    # Process all detected content files
    success_count = 0
    fail_count = 0
    
    for content_type, file_path, issue_num, title in results:
